...
def parse_args():
    ...
    parser.add_argument(
        "--model-path", type=str, default="model/reasoning_model/best_model.h5", help="Chemin vers le modèle entraîné"
    )
    parser.add_argument(
        "--output-dir", type=str, default="results/reasoning", help="Répertoire de sortie pour les résultats"
    )
    parser.add_argument(
        "--num-examples", type=int, default=5, help="Nombre d'exemples à afficher avec des explications"
    )
...
def evaluate_model(model, X_test, y_test):
    ...
    metrics = {"market_regime_accuracy": market_regime_accuracy, "sl_tp_rmse": sl_tp_rmse, "sl_tp_mae": sl_tp_mae}
    ...
    for i in range(num_examples):
        ...
        if reasoning_output is not None and attention_scores is not None:
            ...
            market_regime_explanation = decoder.decode_market_regime_explanation(
                market_regime_classes[i], reasoning_output[i], example_attention, top_k=5
            )

            sl_tp_explanation = decoder.decode_sl_tp_explanation(
                sl_tp_pred[i][0], sl_tp_pred[i][1], reasoning_output[i], reasoning_output[i], example_attention, top_k=5
            )
...
def main():
    ...
    for _, row in explanations_df.iterrows():
        ...
        logger.info(
            f"Prédiction de régime de marché: {row['market_regime_pred']} (confiance: {row['market_regime_confidence']:.2f})"
        )
