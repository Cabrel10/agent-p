...
class ExchangeBase:
        ...
        try:
            ...
            if self.testnet:
                if self.exchange_id == "binance":
                    ...
                    exchange_config["urls"] = {"api": "https://api-testnet.bitget.com"}
    ...
    def _format_symbol(self, symbol: str) -> str:
        ...
        logger.warning(
            f"Aucun format valide trouvé pour le symbole '{symbol}' sur {self.exchange_id}. Utilisation du format original."
        )
        return symbol
    ...
    def _handle_rate_limit(self, e: Exception) -> bool:
        ...
        if self.retry_count <= self.max_retries:
            ...
            logger.warning(
                f"Rate limit atteint. Attente de {delay:.2f}s avant nouvelle tentative ({self.retry_count}/{self.max_retries})"
            )
            time.sleep(delay)
            return True
        else:
