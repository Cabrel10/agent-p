...
class RecoveryManager:
    ...
    def _load_latest_checkpoint(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"État chargé depuis {latest_checkpoint}, dernière mise à jour: {self.trading_state['last_update']}"
            )
            return True
    ...
    def save_checkpoint(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if isinstance(json_state["last_error_time"], datetime):
                json_state["last_error_time"] = (
                    json_state["last_error_time"].isoformat() if json_state["last_error_time"] else None
                )
