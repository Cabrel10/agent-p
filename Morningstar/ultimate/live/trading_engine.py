...
class TradingEngine:
    ...
    def _load_hmm_detector(self, hmm_model_path: Optional[str]) -> Optional[MarketRegimeDetector]:
        ...
        if not hmm_model_path:
            logger.warning(
                "Aucun chemin de modu00e8le HMM spu00e9cifiu00e9. Ru00e9gimes de marchu00e9 non disponibles."
            )
            return None
    ...
    def _prepare_features(self, ohlcv_data: pd.DataFrame, symbol: str) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            features = {
                "technical_input": df_features.iloc[-1][
                    df_features.columns.difference(["timestamp", "open", "high", "low", "close", "volume"])
    ...
    def _execute_signal(self, symbol: str, signal: Dict[str, Any], current_price: float) -> Dict[str, Any]:
        ...
        if signal["confidence"] < min_confidence:
            logger.info(
                f"Signal trop faible ({signal['confidence']:.2f} < {min_confidence}) pour {symbol}. Pas d'action."
            )
    ...
    def run_single_iteration(self) -> Dict[str, Any]:
        ...
        for symbol in self.pairs:
            try:
                ...
                if not features:
                    logger.warning(
                        f"Impossible de pru00e9parer les features pour {symbol}. Passage au symbole suivant."
                    )
                ...
                self.signals_history[symbol] = {"timestamp": datetime.datetime.now().isoformat(), "signal": signal}
