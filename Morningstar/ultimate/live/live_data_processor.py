...
class LiveDataProcessor:
    ...
    def __init__(self, model_dir: Path, config: Dict = None, cache_dir: Optional[Path] = None):
        ...
        self.data_handler = LiveDataHandler(
            model_dir=self.model_dir, scaler_path=self.model_dir / "metadata" / "feature_scaler.pkl"
        )
    ...
    def add_market_regime(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(data) < self.min_history_size:
                logger.warning(
                    f"Données insuffisantes pour la détection de régime: {len(data)} < {self.min_history_size}"
                )
    ...
    def process_live_data(
        self, market_data: pd.DataFrame, sentiment_data: Optional[pd.DataFrame] = None, symbol: Optional[str] = None
    ) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            if "timestamp" not in market_data.columns:
                if "time" in market_data.columns:
                    ...
                    market_data["timestamp"] = pd.date_range(end=datetime.now(), periods=len(market_data), freq="1min")
    ...
    def post_process_predictions(
        self, predictions: Dict[str, np.ndarray], live_data: pd.DataFrame
    ) -> Dict[str, np.ndarray]:
        """
