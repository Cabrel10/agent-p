...
class LiveReasoningIntegration:
    ...
    def generate_explanation(
        self, market_data: pd.DataFrame, predictions: Dict[str, np.ndarray], symbol: str, timeframe: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            explanation = self.reasoning_module.generate_chain_of_thought_explanation(
                market_data=market_data, predictions=predictions, attention_scores=predictions.get("attention_scores")
            )
    ...
    def _get_signal_from_predictions(self, predictions: Dict[str, np.ndarray]) -> str:
        ...
        if "signal_class" in predictions:
            signal_class = (
                predictions["signal_class"][-1]
                if isinstance(predictions["signal_class"], np.ndarray)
                else predictions["signal_class"]
            )
        elif "signal" in predictions:
            signal_class = (
                np.argmax(predictions["signal"][-1])
                if isinstance(predictions["signal"], np.ndarray)
                else np.argmax(predictions["signal"])
            )
    ...
    def _get_confidence_from_predictions(self, predictions: Dict[str, np.ndarray]) -> float:
        ...
        if "signal_confidence" in predictions:
            return (
                float(predictions["signal_confidence"][-1])
                if isinstance(predictions["signal_confidence"], np.ndarray)
                else float(predictions["signal_confidence"])
            )
        elif "signal" in predictions:
            signal_probs = (
                predictions["signal"][-1] if isinstance(predictions["signal"], np.ndarray) else predictions["signal"]
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Intégration du raisonnement Chain-of-Thought pour le trading en direct"
    )
