...
class MetricsLogger:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.order_error_counter = Counter(
            "morningstar_order_error_total", "Number of failed orders", ["symbol", "error_type"]
        )
        self.pnl_gauge = Gauge(
            "morningstar_current_pnl", "Estimated profit & loss", ["symbol"]  # Nom simplifié  # Ajouter le label symbol
        )
        ...
        self.trade_attempts = Counter(
            "morningstar_trade_attempts_total", "Nombre total de tentatives de trade", ["symbol", "side"]
        )
        # Ajouter un compteur pour les prédictions
        self.predictions_received = Counter(
            "morningstar_predictions_received_total", "Nombre total de prédictions reçues du modèle", ["symbol"]
        )
        # Ajouter un compteur pour les erreurs générales
        self.errors_logged = Counter(
            "morningstar_errors_logged_total", "Nombre total d'erreurs système logguées", ["component", "error_type"]
        )
        ...
        try:
            ...
            logger.error(
                f"Impossible de démarrer le serveur Prometheus sur le port {self.prometheus_port}: {e}. Le port est peut-être déjà utilisé."
            )
