...
def parse_args():
    ...
    parser.add_argument(
        "--google-api-key-1", type=str, default="AIzaSyD6NcyTvM73dLJupxS5NFGvv5AtaWhbifU", help="Clu00e9 API Google 1"
    )
    parser.add_argument(
        "--google-api-key-2", type=str, default="AIzaSyCH9ocKRxb_4l7AcdKbzPJAAV1xZ1s-tMQ", help="Clu00e9 API Google 2"
    )
    parser.add_argument(
        "--google-api-key-3", type=str, default="AIzaSyAW7_-kTU1EH8dMXv0oBeDt49mH6lmkZhg", help="Clu00e9 API Google 3"
    )
    parser.add_argument("--gemini-api-key", type=str, default="", help="Clu00e9 API Gemini (optionnelle)")
    parser.add_argument("--exchange-api-key", type=str, default="", help="Clu00e9 API de l'u00e9change (optionnelle)")
    parser.add_argument("--exchange-api-secret", type=str, default="", help="Secret API de l'u00e9change (optionnelle)")
    parser.add_argument(
        "--use-mock-exchange", action="store_true", help="Utiliser un u00e9change simulu00e9 pour les tests"
    )
...
"""
    ...
    logger.info(
        f"- COINMARKETCAP_API_KEY: {'*' * 8 + args.coinmarketcap_api_key[-4:] if args.coinmarketcap_api_key else 'Non configurée'}"
    )
    logger.info(
        f"- GOOGLE_API_KEY_1: {'*' * 8 + args.google_api_key_1[-4:] if args.google_api_key_1 else 'Non configurée'}"
    )
    logger.info(
        f"- GOOGLE_API_KEY_2: {'*' * 8 + args.google_api_key_2[-4:] if args.google_api_key_2 else 'Non configurée'}"
    )
    logger.info(
        f"- GOOGLE_API_KEY_3: {'*' * 8 + args.google_api_key_3[-4:] if args.google_api_key_3 else 'Non configurée'}"
    )
...
def main():
    ...
    logger.info(
        "Vous pouvez maintenant exu00e9cuter les scripts de collecte de donnu00e9es et d'entrau00eenement du modu00e8le"
    )
