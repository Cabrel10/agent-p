...
def main():
    parser = argparse.ArgumentParser(
        description="Amu00e9liore le modu00e8le Morningstar avec des techniques avancu00e9es."
    )
    parser.add_argument(
        "--input-data",
        type=str,
        default="/home/morningstar/Desktop/crypto_robot/Morningstar/data/processed/multi_crypto_dataset_prepared.parquet",
        help="Chemin vers le dataset d'entru00e9e",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/home/morningstar/Desktop/crypto_robot/Morningstar/model/improved",
        help="Ru00e9pertoire de sortie pour les ru00e9sultats",
    )
    parser.add_argument(
        "--skip-data-prep", action="store_true", help="Sauter l'u00e9tape de pru00e9paration des donnu00e9es"
    )
    parser.add_argument(
        "--skip-hyperopt", action="store_true", help="Sauter l'u00e9tape d'optimisation des hyperparamu00e8tres"
    )
    ...
    parser.add_argument(
        "--population-size", type=int, default=20, help="Taille de la population pour l'algorithme gu00e9nu00e9tique"
    )
    parser.add_argument(
        "--generations", type=int, default=10, help="Nombre de gu00e9nu00e9rations pour l'algorithme gu00e9nu00e9tique"
    )
    ...
    normalized_data_path = os.path.join(
        Path(args.input_data).parent, "normalized", Path(args.input_data).stem + "_normalized.csv"
    )
    if not args.skip_data_prep:
        logger.info(
            "u00c9tape 1: Pru00e9paration des donnu00e9es avec normalisation robuste et u00e9quilibrage des classes"
        )
