...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="results/explanations", help="Ru00e9pertoire de sortie pour les explications"
    )
    ...
    for i in range(len(df)):
        ...
        if reasoning_output is not None and attention_scores is not None:
            ...
            try:
                ...
                example_dict["market_regime_explanation"] = (
                    f"Le modu00e8le pru00e9dit un marchu00e9 {market_regime_names[market_regime_classes[i]]}"
                )
...
def main():
    ...
    for i, (_, row) in enumerate(explanations_df.head(3).iterrows()):
        ...
        logger.info(
            f"Pru00e9diction de ru00e9gime de marchu00e9: {row['market_regime_pred']} (confiance: {row['market_regime_confidence']:.2f})"
        )
