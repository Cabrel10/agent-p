    ...
    try:
        ...
        if sentiment_path:
            ...
            if (
                ...
                and "timestamp" in sentiment_data.columns
                and not pd.api.types.is_datetime64_any_dtype(sentiment_data["timestamp"])
            ):
...
def run_backtest(args):
    ...
    try:
        ...
        logger.info(
            f"Données chargées: {len(market_data)} points de marché, "
            + (
                f"{len(sentiment_data)} points de sentiment"
                if sentiment_data is not None
                else "pas de données de sentiment"
            )
        )
        ...
        output_dir = (
            ...
            if args.output_dir
            else Path(f"results/backtest_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        )
        ...
        metrics = engine.run_backtest(market_data, sentiment_data, args.symbol, args.timeframe)
