...
class LLMIntegration:
    ...
    def save_embedding_to_cache(
        self, symbol: str, date_str: str, embedding: np.ndarray, summary: str = "", embedding_type: str = "news"
    ) -> bool:
        """
    ...
    def get_embedding(
        ...
        """
        result = {"embedding": None, "source": "none", "summary": ""}
        ...
        if self.cryptobert_available:
            ...
            if cryptobert_embedding is not None:
                ...
                self.save_embedding_to_cache(
                    symbol, date_str, cryptobert_embedding, summary=result["summary"], embedding_type=embedding_type
                )
        ...
        if self.api_key:
            ...
            if gemini_embedding is not None:
                ...
                # Sauvegarder dans le cache
                self.save_embedding_to_cache(
                    symbol, date_str, gemini_embedding, summary=result["summary"], embedding_type=embedding_type
                )
        ...
        logger.warning(
            f"Aucune méthode d'embedding disponible. Génération d'un embedding aléatoire pour {symbol}-{date_str}."
        )
        ...
        self.save_embedding_to_cache(
            symbol, date_str, random_embedding, summary=result["summary"], embedding_type=embedding_type
        )
