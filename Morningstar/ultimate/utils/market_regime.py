...
class MarketRegimeDetector:
    ...
    def _prepare_features(self, data: pd.DataFrame) -> np.ndarray:
        ...
        features_dict = {"returns": returns, "volatility": volatility, "sharpe_ratio": sharpe_ratio}
    ...
    def _analyze_regimes(self, data: pd.DataFrame, scaled_features: np.ndarray) -> None:
        ...
        for regime, stats in regime_stats.items():
            logger.info(
                f"Régime {regime}: {stats['count']} points ({stats['percentage']:.1f}%), "
    ...
    def get_regime_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {"regimes": regimes, "probabilities": probabilities, "descriptions": regime_descriptions}
