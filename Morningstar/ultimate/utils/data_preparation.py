    ...
    try:
        ...
        if input_path.endswith(".csv"):
            ...
            if date_col:
                ...
                logger.warning(
                    "No standard date/timestamp column found ('timestamp', 'date', 'Date'). Index will be RangeIndex."
                )
...
def prepare_link_4h_dataset(force_download: bool = False) -> pd.DataFrame:
    ...
    for i in range(128):
        df[f"mcp_{i}"] = (
            np.random.normal(mcp_features[i], 0.1, len(df))
            if i < len(mcp_features)
            else np.random.normal(0, 0.1, len(df))
        )
