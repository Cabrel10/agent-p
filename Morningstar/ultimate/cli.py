...
try:
    ...
    try:
        ...
        psutil = None
        print(
            "Avertissement: Le module 'psutil' n'est pas installé. Les commandes 'live stop' et 'live status' pourraient être moins robustes."
        )
    ...
    try:
        ...
        print(
            "Avertissement: Module 'reporting.reporting_utils' non trouvé. Les commandes 'report' ne fonctionneront pas."
        )
    ...
    try:
        ...
        print(
            "Avertissement: Module 'utils.maintenance' non trouvé. La commande 'utils clean-cache' ne fonctionnera pas."
        )
    # ... etc ...
...
global_settings: Dict[str, Any] = {"verbose": False, "dry_run": False, "config_path": CONFIG_PATH}
        ...
        try:
            if action == "1":
                new_exchange = questionary.text(
                    "Nouvel Exchange (ex: binance, kucoin):", default=current_exchange
                ).ask()
                if new_exchange:
                ...
                new_symbol = questionary.text(
                    "Nouveau Symbole (ex: ETH/USDT):", default=current_symbol, validate=is_valid_asset
                ).ask()
                if new_symbol:
                ...
                new_strategy = questionary.text(
                    "Nouvelle stratégie (ex: live_strategy, scalping):", default=current_strategy
                ).ask()
                if new_strategy:
        ...
        try:
            if action == "1":
                new_asset = questionary.text(
                    "Nouvel Asset (ex: ETH/USDT):", default=current_asset, validate=is_valid_asset
                ).ask()
                if new_asset:
                    current_asset = new_asset.upper()
            elif action == "2":
                new_tf = questionary.text(
                    "Nouveau Timeframe (ex: 4h):", default=current_tf, validate=is_valid_timeframe
                ).ask()
                if new_tf:
                    current_tf = new_tf.lower()
            elif action == "3":
                new_start = questionary.text(
                    "Nouvelle Date de Début (YYYY-MM-DD):", default=current_start, validate=is_valid_date
                ).ask()
                if new_start:
                    current_start = new_start
            elif action == "4":
                new_end = questionary.text(
                    "Nouvelle Date de Fin (YYYY-MM-DD):", default=current_end, validate=is_valid_date
                ).ask()
                if new_end:
                    current_end = new_end
                ...
                success = data_pipeline(asset=current_asset, limit=limit, dry_run=global_settings["dry_run"])
                if success:
                ...
                try:
                    ...
                    split_ratios = questionary.text(
                        ...
                        validate=lambda text: all([r.isdigit() for r in text.split(",")])
                        and sum(map(int, text.split(","))) == 100,
                    ).ask()
                    train_ratio, val_ratio, test_ratio = map(int, split_ratios.split(","))
                    success = split_dataset(
                        asset=current_asset, train_ratio=train_ratio, val_ratio=val_ratio, test_ratio=test_ratio
                    )
                    if success:
    ...
    while True:
        ...
        try:
            if action == "1":
                ...
                new_strategy = questionary.text(
                    "Nouvelle stratégie (ex: simple, advanced):", default=current_strategy
                ).ask()
                if new_strategy:
                    current_strategy = new_strategy
        ...
        try:
            if action == "1":
                new_epochs = questionary.text(
                    "Nombre d'époques:", default=current_epochs, validate=lambda text: text.isdigit() and int(text) > 0
                ).ask()
                if new_epochs:
                    current_epochs = new_epochs
            elif action == "2":
                new_batch = questionary.text(
                    "Taille du batch:", default=current_batch, validate=lambda text: text.isdigit() and int(text) > 0
                ).ask()
                if new_batch:
                ...
                try:
                    ...
                    success = run_hyperparameter_tuning(
                        config_path=global_settings["config_path"], dry_run=global_settings["dry_run"]
                    )
                    if success:
                ...
                try:
                    ...
                    plot_training_curves(config_path=global_settings["config_path"])
                except ImportError:
...
def data_ingest(
    ...
    if fetch_ohlcv_data is None or save_data is None or format_symbol is None:
        console.print(
            "[bold red]Erreur: Les fonctions nécessaires depuis utils.api_manager n'ont pas pu être importées.[/]"
        )
        return False  # Indique l'échec
    ...
    try:
        ...
        with Progress(console=console) as progress:
            ...
            progress.update(
                task, completed=100, description=f"[cyan]Téléchargement {formatted_asset} terminé."
        ...
        if df_data is not None and not df_data.empty:
            ...
            if save_data(df_data, str(output)):
                ...
                if verify_downloaded_file and verify_downloaded_file(str(output)):
                    ...
                    console.print(
                        "[yellow]Avertissement: La vérification post-sauvegarde a échoué ou n'est pas disponible.[/]"
                    )
            else:
            ...
            console.print(
                f"[bold red]Échec du téléchargement. La fonction fetch_ohlcv_data a retourné None. Vérifiez les logs pour plus de détails.[/]"
            )
            return False  # Indique l'échec
        else:  # df_data is empty
            console.print(
                f"[bold yellow]Aucune donnée retournée par l'exchange pour {formatted_asset} sur cette période/timeframe.[/]"
            )
...
def data_pipeline(
    ...
    limit: Optional[
        int
    ],  # = typer.Option(None, "--limit", "-l", help="Limiter le traitement aux N premières lignes du fichier brut (pour tests)."), # Modifié
    ...
    console.print(
        f"[bold blue]Démarrage du pipeline de données pour {asset}{f' (limité à {limit} lignes)' if limit else ''}[/]"
    )

    if run_pipeline is None:
        console.print(
            "[bold red]Erreur: La fonction 'run_pipeline' n'a pas pu être importée depuis data.pipelines.data_pipeline.[/]"
        )
        return False  # Indique l'échec
    ...
    try:
        ...
        with Progress(console=console) as progress:
            ...
            if not input_file.exists():
                ...
                console.print(
                    f"[bold yellow]Assurez-vous d'avoir exécuté 'data ingest' pour {asset} avec les bons paramètres.[/]"
                )
                return False  # Indique l'échec
            ...
            progress.update(
                task,
                completed=100,
                description=f"[cyan]Pipeline {asset}{f' (limité à {limit} lignes)' if limit else ''} terminé.",
            )
...
def data_validate(
    ...
    if validate_processed_data is None:
        console.print(
            "[bold red]Erreur: La fonction 'validate_processed_data' n'a pas pu être importée depuis data.pipelines.data_pipeline.[/]"
        )
        return False  # Indique l'échec
    ...
    try:
        ...
        is_valid = validate_processed_data(asset=asset, config_path=config_path)  # Utilisation directe
        ...
        console.print(
            f"[bold red]Erreur: Fichier de données traitées pour {asset} non trouvé. Exécutez d'abord 'data pipeline'.[/]"
        )
        return False  # Indique l'échec
    except Exception as e:
...
def train_local(
    ...
    if run_local_training is None:
        console.print(
            "[bold red]Erreur: La fonction 'run_local_training' n'a pas pu être importée depuis model.training.training_script.[/]"
        )
        return False  # Indique l'échec
...
def train_colab(
    ...
    console.print(
        "1. [bold]Uploadez le notebook[/] (si nécessaire) et l'ensemble du projet (ou au moins les données et le code requis) sur votre Google Drive."
    )
    console.print(f"2. [bold]Ouvrez Google Colab[/] et chargez le notebook : [cyan]{notebook.name}[/]")
    console.print(
        "3. [bold]Montez votre Google Drive[/] dans le notebook en exécutant la cellule appropriée (généralement la première)."
    )
    ...
    console.print("   ```")
    console.print(
        "4. [bold]Ajustez les chemins[/] dans le notebook pour pointer vers les fichiers sur votre Drive (ex: `/content/drive/MyDrive/crypto_robot/...`)."
    )
    ...
    console.print(
        "7. Assurez-vous que l'environnement d'exécution Colab utilise un [bold]GPU[/] pour accélérer l'entraînement (Menu 'Exécution' -> 'Modifier le type d'exécution')."
    )
...
def start_live_process(
    ...
    exchange: Optional[
        str
    ],  # = typer.Option(None, help="ID de l'exchange (ex: binance), surcharge la config."), # Modifié
    symbol: Optional[
        str
    ],  # = typer.Option(None, help="Symbole à trader (ex: BTC/USDT), surcharge la config.") # Modifié
    dry_run: bool,  # Ajouté
):
    ...
    if PID_FILE.exists():
        try:
            ...
            if psutil and psutil.pid_exists(pid):
                console.print(
                    f"[bold yellow]Un processus live semble déjà en cours (PID: {pid}). Utilisez 'live status' ou 'live stop'.[/]"
                )
    ...
    try:
        ...
        logging.error(
            "Erreur: 'python' ou 'run_live.py' non trouvé. Assurez-vous d'être dans le bon environnement/répertoire."
        )
...
def stop_live_process(dry_run: bool):  # Ajouté dry_run
    ...
    try:
        ...
        console.print(
            f"[bold red]Erreur: Fichier PID invalide ({PID_FILE}). Contenu: '{pid_str}'. Nettoyage manuel requis.[/]"
        )
        return False  # Indique l'échec
    except Exception as e:
    ...
    if dry_run:  # Utilisation directe
        console.print(
            f"[yellow]DRY-RUN: Simulation de l'arrêt du processus avec PID {pid} et suppression de {PID_FILE}.[/]"
        )
    ...
    try:
        ...
        try:
            ...
            if gone:
                ...
                console.print(
                    f"[yellow]Le processus {pid} n'a pas répondu au SIGTERM après 10s. Envoi de SIGKILL...[/]"
                )
        ...
        console.print(
            f"[bold red]Erreur: Accès refusé pour arrêter le processus {pid}. Exécutez avec les permissions appropriées.[/]"
        )
        return False  # Indique l'échec
    except Exception as e:
...
def get_live_status(dry_run: bool):  # Ajouté dry_run
    ...
    try:
        ...
        console.print(
            f"[bold red]Erreur: Fichier PID invalide ({PID_FILE}). Contenu: '{pid_str}'. Utilisez 'live stop' pour nettoyer si possible.[/]"
        )
    ...
    if dry_run:  # Utilisation directe
        ...
        if pid == 0 and PID_FILE.exists():
            console.print(
                "[yellow]DRY-RUN: Le processus live est simulé comme étant 'en cours' (fichier PID simulé trouvé).[/]"
            )
        else:
            console.print(
                "[yellow]DRY-RUN: Le processus live est simulé comme étant 'arrêté' (pas de fichier PID simulé).[/]"
            )
    ...
    if psutil is None:
        ...
        console.print(
            f"Fichier PID trouvé ({PID_FILE}) avec PID {pid}. Le processus est [bold]probablement[/] en cours."
        )
        return  # Affichage suffit
    ...
    try:
        if psutil.pid_exists(pid):
            ...
            console.print(
                f"[yellow]Statut: Le processus live (PID {pid}) n'est PLUS en cours, mais le fichier PID existe.[/]"
            )
        ...
        console.print(
            f"[yellow]Statut: Fichier PID trouvé (PID {pid}), mais accès refusé pour vérifier l'état réel.[/]"
        )
...
def report_metrics(
    ...
    try:
        ...
        success = load_and_display_metrics(run_id=run_id, config_path=config_path)  # Utilisation directe
...
def report_export(
    ...
    if format.lower() not in allowed_formats:
        console.print(
            f"[bold red]Erreur: Format d'export '{format}' non supporté. Formats valides: {', '.join(allowed_formats)}[/]"
        )
        return False  # Indique l'échec
    ...
    try:
        ...
        result_path = export_run_report(
            run_id=run_id, format=format.lower(), config_path=config_path  # Utilisation directe
        )
...
def utils_clean_cache(dry_run: bool):  # Ajouté dry_run
    ...
    if clean_all_caches is None:
        console.print(
            "[bold red]Erreur: La fonction 'clean_all_caches' n'a pas pu être importée depuis utils.maintenance.[/]"
        )
        return False  # Indique l'échec
...
def handle_utils_menu():
    ...
    while True:
        menu_choice = questionary.select(
            "Choisissez une action:", choices=["1. Clean Cache", "2. ← Back to Main Menu"], qmark="🛠️", pointer="→"
        ).ask()
...
def run_interactive_menu():
    ...
    if not config_data:
        console.print(
            "[bold yellow]Avertissement: Impossible de charger la configuration. Les valeurs par défaut pourraient être incorrectes.[/]"
        )
