...
def data_collection_page():
    ...
    tabs = st.tabs(["Collecte de données complètes", "Visualisation des données"])
        ...
        with st.form("complete_data_form"):
            ...
            with col1:
                ...
                selected_symbols = st.multiselect(
                    "Sélectionner les symboles", AVAILABLE_SYMBOLS, default=["BTC/USDT", "ETH/USDT"]
                )

                # Période de collecte
                start_date = st.date_input(
                    "Date de début", value=datetime.now() - timedelta(days=365), key="complete_start_date"
                )

                end_date = st.date_input("Date de fin", value=datetime.now(), key="complete_end_date")
            ...
            with col2:
                ...
                max_workers = st.slider(
                    "Nombre de workers", min_value=1, max_value=8, value=2, key="complete_max_workers"
                )

                # Utiliser le cache
                use_cache = st.checkbox(
                    "Utiliser le cache", value=True, help="Utiliser les données déjà collectées si disponibles"
                )

            # Nom du dataset
            dataset_name = st.text_input(
                "Nom du dataset", value="complete_dataset", help="Nom du fichier de sortie (sans extension)"
            )
    ...
    with tabs[1]:
        ...
        if datasets:
            selected_dataset = st.selectbox("Sélectionner un dataset pour l'aperçu", datasets)
                ...
                if df is not None:
                    ...
                    if all(col in df.columns for col in ["open", "high", "low", "close"]):
                        ...
                        if "symbol" in df.columns:
                            ...
                            selected_symbol = st.selectbox("Sélectionner un symbole pour le graphique", symbols)
                        ...
                        if "timestamp" in symbol_df.columns:
                            ...
                            with col1:
                                start_date = st.date_input(
                                    "Date de début du graphique", value=min_date, min_value=min_date, max_value=max_date
                                )
                            with col2:
                                end_date = st.date_input(
                                    "Date de fin du graphique", value=max_date, min_value=min_date, max_value=max_date
                                )
                            ...
                            symbol_df = symbol_df[
                                (symbol_df["timestamp"] >= start_datetime) & (symbol_df["timestamp"] <= end_datetime)
                            ]
                        ...
                        if not symbol_df.empty:
                            ...
                            if "volume" in symbol_df.columns:
                                ...
                                volume_colors = [
                                    "red" if row["close"] < row["open"] else "green" for _, row in symbol_df.iterrows()
                                ]
                            ...
                            with col1:
                                price_change = (
                                    (symbol_df["close"].iloc[-1] - symbol_df["close"].iloc[0])
                                    / symbol_df["close"].iloc[0]
                                    * 100
                                )
                        ...
                        st.warning(
                            "Le dataset ne contient pas les colonnes OHLC nécessaires pour afficher un graphique des prix."
                        )
        else:
            st.info(
                "Aucun dataset disponible. Utilisez l'onglet 'Collecte de données complètes' pour collecter des données."
            )
