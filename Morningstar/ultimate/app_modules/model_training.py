...
def model_training_page():
    ...
    tabs = st.tabs(["Entraînement standard", "Optimisation génétique"])
        ...
        with st.form("morningstar_model_form"):
            ...
            with col2:
                ...
                optimizer = st.selectbox(
                    "Optimiseur", ["Adam", "RMSprop", "SGD", "AdamW"], index=0, help="Algorithme d'optimisation"
                )
            ...
            with st.expander("Paramètres avancés"):
                ...
                with col1:
                    ...
                    use_batch_norm = st.checkbox(
                        "Utiliser Batch Normalization", value=True, help="Activer la normalisation par batch"
                    )
                ...
                with col2:
                    ...
                    use_chain_of_thought = st.checkbox(
                        "Activer Chain-of-Thought", value=True, help="Activer le raisonnement Chain-of-Thought"
                    )
            ...
            with col1:
                model_name = st.text_input("Nom du modèle", value="morningstar", help="Nom du modèle à sauvegarder")
    ...
    if submit_button:
        if selected_dataset == "Aucun dataset disponible":
            ...
            args = [
                ...
                str(tech_input_dim),
                "--llm-embedding-dim",
                str(llm_embedding_dim),
                "--mcp-input-dim",
                str(mcp_input_dim),
                "--hmm-input-dim",
                str(hmm_input_dim),
                "--num-trading-classes",
                str(num_trading_classes),
                "--num-market-regime-classes",
                str(num_market_regime_classes),
                "--num-volatility-quantiles",
                str(num_volatility_quantiles),
                "--num-sl-tp-outputs",
                str(num_sl_tp_outputs),
                "--num-reasoning-steps",
                str(num_reasoning_steps),
                "--num-attention-heads",
        ...
        with st.form("genetic_optimization_form"):
            ...
            with col1:
                l2_reg_min = st.number_input(
                    "L2 régularisation (min)", min_value=0.0001, max_value=0.01, value=0.0001, format="%.4f"
                )

                l2_reg_max = st.number_input(
                    "L2 régularisation (max)", min_value=0.0001, max_value=0.01, value=0.01, format="%.4f"
                )

            with col2:
                dropout_min = st.number_input("Dropout (min)", min_value=0.1, max_value=0.5, value=0.1, step=0.1)

                dropout_max = st.number_input("Dropout (max)", min_value=0.1, max_value=0.5, value=0.5, step=0.1)

            with col3:
                lr_min = st.number_input(
                    "Taux d'apprentissage (min)", min_value=0.0001, max_value=0.01, value=0.0001, format="%.4f"
                )

                lr_max = st.number_input(
                    "Taux d'apprentissage (max)", min_value=0.0001, max_value=0.01, value=0.01, format="%.4f"
                )
            ...
            with col1:
                batch_size_min = st.select_slider("Taille du batch (min)", options=[16, 32, 64, 128], value=16)

                batch_size_max = st.select_slider("Taille du batch (max)", options=[16, 32, 64, 128], value=128)
            ...
            if selected_dataset == "Aucun dataset disponible":
                ...
                if l2_reg_min >= l2_reg_max:
                    ...
                    args = [
                        ...
                        selected_dataset,
                        "--output-dir",
                        str(MODEL_DIR / "trained" / "morningstar"),
                        "--population-size",
                        str(population_size),
                        "--generations",
                        str(generations),
                            ...
                            if results_path.exists():
                                ...
                                for i, (param, value) in enumerate(best_hyperparams.items()):
                                    with cols[i]:
                                        ...
                                        st.metric(label=param.replace("_", " ").title(), value=value)
    ...
    if models:
        ...
        for model_path in models:
            ...
            model_data.append({"Nom": model_name, "Taille": f"{model_size:.2f} Mo", "Date de création": model_date})
