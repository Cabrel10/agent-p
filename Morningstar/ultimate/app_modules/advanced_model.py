...
def run_advanced_script(script_name, script_args):
    """Exécute un script Python en tant que sous-processus."""
    try:
        script_path = (
            Path(BASE_DIR) / "scripts" / script_name
        )  # Assumant que les scripts sont dans un dossier scripts/ à la racine
        if not script_path.exists():
            # Essayer dans le dossier model/training/ si non trouvé
            script_path = Path(MODEL_DIR) / "training" / script_name
            if not script_path.exists():
                # Essayer directement à la racine du projet pour les scripts comme genetic_optimizer.py
                script_path = Path(BASE_DIR) / script_name
                if not script_path.exists():
                    return False, f"Script {script_name} non trouvé."

        command = [sys.executable, str(script_path)] + script_args
        st.info(f"Exécution de la commande : {' '.join(command)}")

        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

        # Afficher la sortie en temps réel (optionnel, peut être verbeux)
        stdout_lines = []
        stderr_lines = []

        # Utiliser des threads pour lire stdout et stderr sans bloquer
        def read_pipe(pipe, lines_list):
            for line in iter(pipe.readline, ""):
                st.text(line.strip())  # Afficher en temps réel dans Streamlit
        ...
        return False, f"Exception lors de l'exécution de {script_name}: {str(e)}"
...
def advanced_model_page():
    ...
    tabs = st.tabs(
        ["Architecture du modèle", "Optimisation génétique", "Entraînement avancé", "Visualisation des résultats"]
    )
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            for input_name, dim in MODEL_DIMENSIONS.items():
                input_data.append({"Entrée": input_name, "Dimensions": dim})
        ...
        with col2:
            ...
            output_data = [
                ...
                {
                    "Sortie": "volatility_quantiles",
                    "Dimensions": 3,
                    "Description": "Quantiles de volatilité (3 valeurs)",
                },
                {
                    "Sortie": "stop_loss_take_profit",
                    "Dimensions": 2,
                    "Description": "Stop Loss et Take Profit (2 valeurs)",
                },
            ]
        ...
        with st.form("genetic_optimization_form"):
            ...
            with col2:
                ...
                selected_dataset = st.selectbox(
                    "Sélectionner le dataset", datasets if datasets else ["Aucun dataset disponible"]
                )
            ...
            with col1:
                learning_rate_min = st.number_input(
                    "Learning rate (min)", min_value=0.0001, max_value=0.01, value=0.0001, format="%.4f"
                )

                learning_rate_max = st.number_input(
                    "Learning rate (max)", min_value=0.001, max_value=0.1, value=0.01, format="%.4f"
                )

            with col2:
                batch_size_min = st.number_input("Batch size (min)", min_value=16, max_value=64, value=32, step=16)

                batch_size_max = st.number_input("Batch size (max)", min_value=64, max_value=256, value=128, step=32)

            with col3:
                epochs_min = st.number_input("Epochs (min)", min_value=10, max_value=50, value=20, step=10)

                epochs_max = st.number_input("Epochs (max)", min_value=50, max_value=200, value=100, step=10)
        ...
        if submit_button:
            if selected_dataset == "Aucun dataset disponible":
                ...
                args = [
                    ...
                    str(elite_size),
                    "--output-report",
                    str(REPORTS_DIR / "genetic_optimization_results.json"),
                ]
                ...
                if success:
                    ...
                    for param, value in best_params.items():
                        best_params_data.append({"Paramètre": param, "Valeur": value})
                    ...
                    fig = go.Figure()
                    fig.add_trace(
                        go.Scatter(x=generations_list, y=best_fitness, mode="lines+markers", name="Meilleure fitness")
                    )
                    fig.add_trace(
                        go.Scatter(x=generations_list, y=avg_fitness, mode="lines+markers", name="Fitness moyenne")
                    )
    ...
    with tabs[2]:
        ...
        with st.form("advanced_training_form"):
            ...
            with col1:
                ...
                epochs = st.slider(
                    "Nombre d'époques", min_value=10, max_value=200, value=50, step=10, key="advanced_epochs"
                )

                batch_size = st.slider(
                    "Taille du batch", min_value=16, max_value=256, value=64, step=16, key="advanced_batch_size"
                )
            ...
            with col2:
                ...
                # Nom du modèle
                model_name = st.text_input("Nom du modèle", value="hybrid_model", key="advanced_model_name")
            ...
            with col1:
                dropout_rate = st.slider("Taux de dropout", min_value=0.0, max_value=0.5, value=0.2, step=0.1)
        ...
        if submit_button:
            if selected_dataset == "Aucun dataset disponible":
                ...
                if success:
                    ...
                    fig1 = go.Figure()
                    fig1.add_trace(go.Scatter(x=epochs_list, y=train_loss, mode="lines", name="Perte d'entraînement"))
                    fig1.add_trace(go.Scatter(x=epochs_list, y=val_loss, mode="lines", name="Perte de validation"))
                    ...
                    fig2 = go.Figure()
                    fig2.add_trace(
                        go.Scatter(x=epochs_list, y=train_acc, mode="lines", name="Précision d'entraînement")
                    )
                    fig2.add_trace(go.Scatter(x=epochs_list, y=val_acc, mode="lines", name="Précision de validation"))
                    ...
                    for metric, value in metrics.items():
                        metrics_data.append({"Métrique": metric, "Valeur": value})
    ...
    with tabs[3]:
        ...
        with col1:
            ...
            if hybrid_models:
                selected_model = st.selectbox("Sélectionner le modèle hybride", hybrid_models)
            else:
                selected_model = st.selectbox("Sélectionner le modèle hybride", ["Aucun modèle hybride disponible"])
        ...
        selected_idx = st.number_input(
            "Sélectionner l'index de la donnée à visualiser/expliquer",
            min_value=0,
            value=0,
            step=1,
            key="selected_idx_viz",
        )
        ...
        if st.button("Générer les prédictions"):
            if selected_model == "Aucun modèle hybride disponible":
                ...
                df = pd.DataFrame({"date": dates, "price": prices, "signal": signals})
                ...
                if explanation_available:
                    ...
                    if "signal" in df.columns:
                        signal_idx = (
                            int(df.loc[selected_idx, "signal"])
                            if pd.notna(df.loc[selected_idx, "signal"])
                            else selected_idx % 5
                        )
                    ...
                    if "market_regime" in df.columns:
                        regime_idx = (
                            int(df.loc[selected_idx, "market_regime"])
                            if pd.notna(df.loc[selected_idx, "market_regime"])
                            else selected_idx % 4
                        )
                    ...
                    if "level_sl" in df.columns and "level_tp" in df.columns:
                        sl_value = (
                            df.loc[selected_idx, "level_sl"] if pd.notna(df.loc[selected_idx, "level_sl"]) else -0.02
                        )
                        tp_value = (
                            df.loc[selected_idx, "level_tp"] if pd.notna(df.loc[selected_idx, "level_tp"]) else 0.04
                        )
                    else:
                ...
                if not attention_available:
                    ...
                    features = [
                        ...
                        "Bollinger Bands",
                        "Sentiment Gemini",
                        "Embedding CryptoBERT 1",
                        "Embedding CryptoBERT 2",
...
def advanced_model_page():
    ...
    Affiche la page du modèle avancé.
    """
    # model_architecture_page() # Commenté car l'architecture est dans le premier onglet
