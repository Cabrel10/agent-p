...
def model_evaluation_page():
    ...
    if morningstar_exists:
        ...
        st.warning(
            "⚠️ Le modèle Morningstar n'est pas disponible. Veuillez le créer en exécutant le script `scripts/create_simple_model.py`."
        )

    # Créer des onglets pour les différentes sections
    tabs = st.tabs(["Évaluation individuelle", "Comparaison de modèles", "Backtesting", "Chain-of-Thought"])
        ...
        with st.form("individual_evaluation_form_tab"):
            ...
            with col1:
                ...
                selected_model = st.selectbox(
                    "Sélectionner le modèle à évaluer", models if models else ["Aucun modèle disponible"]
                )
            ...
            with col2:
                ...
                selected_dataset = st.selectbox(
                    "Sélectionner le dataset de test", datasets if datasets else ["Aucun dataset disponible"]
                )
        ...
        if submit_button:
            if selected_model == "Aucun modèle disponible":
                ...
                args = [
                    ...
                    selected_dataset,
                    "--output-report",
                    str(REPORTS_DIR / f"evaluation_{Path(selected_model).stem}_{Path(selected_dataset).stem}.json"),
                ]
                ...
                if success:
                    ...
                    with col4:
                        create_metric_card(
                            "Drawdown max",
                            f"{evaluation_results.get('max_drawdown', 0)*100:.1f}%",
                            delta_color="inverse",
                        )
        ...
        if submit_button:
            if not selected_models or selected_models[0] == "Aucun modèle disponible":
                ...
                args = [
                    ...
                    selected_dataset,
                    "--output-report",
                    str(REPORTS_DIR / f"comparison_{Path(selected_dataset).stem}.json"),
                ]
                ...
                if success:
                    ...
                    for metric, metric_name in zip(metrics, metric_names):
                        ...
                        metric_values = [
                            get_evaluation_results(model, selected_dataset).get(metric, 0) for model in selected_models
                        ]
    ...
    with tabs[2]:
        ...
        with st.form("backtest_form"):
            ...
            with col2:
                ...
                take_profit = st.slider("Take Profit (%)", min_value=0.5, max_value=10.0, value=3.0, step=0.5)

                stop_loss = st.slider("Stop Loss (%)", min_value=0.5, max_value=10.0, value=2.0, step=0.5)

                # Capital initial
                initial_capital = st.number_input(
                    "Capital initial ($)", min_value=100, max_value=1000000, value=10000, step=1000
                )
        ...
        if submit_button:
            if selected_model == "Aucun modèle disponible":
                ...
                args = [
                    ...
                    str(initial_capital),
                    "--output-report",
                    str(REPORTS_DIR / f"backtest_{Path(selected_model).stem}_{Path(selected_dataset).stem}.json"),
                ]
                ...
                if success:
                    ...
                    backtest_report_path = (
                        REPORTS_DIR / f"backtest_{Path(selected_model).stem}_{Path(selected_dataset).stem}.json"
                    )
                    ...
                    if backtest_report_path.exists():
                        try:
                            ...
                            with col1:
                                create_metric_card(
                                    "Rendement total", f"{backtest_results.get('total_return', 0)*100:.1f}%"
                                )

                            with col2:
                                create_metric_card(
                                    "Facteur de profit", format_number(backtest_results.get("profit_factor", 0))
                                )

                            with col3:
                                create_metric_card(
                                    "Ratio de Sharpe", format_number(backtest_results.get("sharpe_ratio", 0))
                                )

                            with col4:
                                create_metric_card(
                                    "Drawdown max",
                                    f"{backtest_results.get('max_drawdown', 0)*100:.1f}%",
                                    delta_color="inverse",
                                )
                            ...
                            with col1:
                                create_metric_card(
                                    "Taux de réussite", f"{backtest_results.get('win_rate', 0)*100:.1f}%"
                                )

                            with col2:
                                create_metric_card(
                                    "Profit moyen", f"${format_number(backtest_results.get('avg_profit', 0))}"
                                )

                            with col3:
                                create_metric_card(
                                    "Perte moyenne", f"${format_number(backtest_results.get('avg_loss', 0))}"
                                )
    ...
    with tabs[3]:
        st.markdown(
            "<h2 class='sub-header'>Visualisation du raisonnement Chain-of-Thought</h2>", unsafe_allow_html=True
        )
        ...
        if not morningstar_exists:
            ...
            st.info(
                "Cette section permet de visualiser les explications générées par le modèle Morningstar pour ses décisions de trading."
            )

            # Sélection du dataset
            datasets = get_available_datasets()
            selected_dataset = st.selectbox(
                "Dataset à utiliser", datasets if datasets else ["Aucun dataset disponible"], key="cot_dataset"
            )
                            ...
                            if model is None:
                                ...
                                if dataset is None or len(dataset) == 0:
                                    ...
                                    inputs = {
                                        ...
                                        "instrument_input": np.zeros(
                                            (sample_size, 1), dtype=np.int32
                                        ),  # Placeholder pour l'identifiant d'instrument
                                    }
                                        ...
                                        with st.container():
                                            ...
                                            with col1:
                                                ...
                                                if "market_regime" in predictions:
                                                    ...
                                                    regime_name = (
                                                        regime_names[market_regime]
                                                        if market_regime < len(regime_names)
                                                        else f"Régime {market_regime}"
                                                    )
