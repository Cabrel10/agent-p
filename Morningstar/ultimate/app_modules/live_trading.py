...
AVAILABLE_EXCHANGES = ["binance", "bitget", "kucoin", "bybit", "huobi", "okx", "bitfinex", "kraken", "coinbase"]

# Modes de trading
TRADING_MODES = ["Simulation", "Papier", "Réel"]
...
class TradingBot:
    ...
    def _init_exchange(self):
        ...
        try:
            if self.mode == "Simulation":
                ...
                config = {"api_key": self.api_key, "api_secret": self.api_secret, "testnet": self.use_testnet}
                ...
                self.exchange_manager = ExchangeManager(exchange_id=self.exchange_id, use_testnet=self.use_testnet)
    ...
    def _execute_trade(self, symbol, action, amount=None, price=None):
        ...
        if not self.trading_active:
            return {"success": False, "message": "Trading désactivé. Aucun ordre n'a été exécuté."}
        ...
        try:
            ...
            if not current_price:
                ...
                return {"success": False, "message": f"Impossible d'obtenir le prix actuel pour {symbol}"}
            ...
            if self.mode == "Simulation":
                ...
                if action == "buy":
                    ...
                    if self.balance.get(quote_currency, 0) >= trade_result["value"]:
                        ...
                        return {"success": False, "message": f"Fonds insuffisants en {quote_currency}"}
                else:  # sell
                    base_currency = symbol.split("/")[0]
                    quote_currency = symbol.split("/")[1]
                    ...
                    if self.balance.get(base_currency, 0) >= amount:
                        ...
                        return {"success": False, "message": f"Actifs insuffisants en {base_currency}"}
                ...
                if not self.exchange_manager:
                    return {"success": False, "message": "Gestionnaire d'échange non initialisé"}
                ...
                try:
                    ...
                    return {"success": False, "message": f"Erreur lors de l'exécution de l'ordre: {str(e)}"}
            ...
            return {"success": False, "message": "Échec de l'exécution du trade pour une raison inconnue"}
        except Exception as e:
            self._handle_error(f"Erreur lors de l'exécution du trade: {str(e)}")
            return {"success": False, "message": f"Erreur lors de l'exécution du trade: {str(e)}"}
...
def show_live_trading():
    ...
    if not st.session_state.trading_bot:
        ...
        if latest_checkpoint and not st.session_state.trading_bot:
            st.info(
                f"Un checkpoint du {latest_checkpoint['timestamp']} a été trouvé. Voulez-vous restaurer l'état précédent du bot de trading?"
            )
        ...
        with st.form("trading_config_form"):
            ...
            with col1:
                ...
                exchange_id = st.selectbox("Échange", AVAILABLE_EXCHANGES, index=0)

                # Sélection du modèle
                models = get_available_models()
                selected_model = st.selectbox(
                    "Modèle à utiliser", models if models else ["Aucun modèle disponible"], key="trading_model"
                )

                # Sélection du mode de trading
                trading_mode = st.selectbox("Mode de trading", TRADING_MODES, index=0)
            ...
            with col2:
                ...
                selected_symbols = st.multiselect("Symboles à trader", AVAILABLE_SYMBOLS, default=["BTC/USDT"])

                # Clés API (uniquement pour les modes Papier et Réel)
                api_key = st.text_input("Clé API", type="password", disabled=trading_mode == "Simulation")

                api_secret = st.text_input("Secret API", type="password", disabled=trading_mode == "Simulation")

                # Mot de passe/Passphrase (pour certains échanges comme Bitget)
                if exchange_id.lower() == "bitget":
                    api_passphrase = st.text_input(
                        "Passphrase API", type="password", disabled=trading_mode == "Simulation"
                    )
        ...
        if submit_button:
            if selected_model == "Aucun modèle disponible":
                ...
                st.success(
                    f"Bot de trading configuré avec succès pour {len(selected_symbols)} symboles en mode {trading_mode}."
                )
