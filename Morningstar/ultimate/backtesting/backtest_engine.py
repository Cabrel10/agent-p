...
class BacktestEngine:
    ...
    def __init__(self, model_path: Union[str, Path], config: Optional[Dict[str, Any]] = None):
        ...
        self.data_processor = LiveDataProcessor(
            model_dir=self.model_path, config=self.config, cache_dir=self.model_path / "cache"
        )
    ...
    def run_backtest(
        ...
        position = 0.0
        entry_price = 0.0
        entry_time = None  # Initialisation de entry_time
        equity_curve = []
        trades = []
        current_sl = None
        current_tp = None
        ...
        for i in tqdm(range(window_size, len(data)), desc="Backtesting"):
            ...
            try:
                ...
                reasoning = None
                if self.reasoning_module is not None and hasattr(
                    self.reasoning_module, "generate_chain_of_thought_explanation"
                ):
                    try:
                ...
                if position != 0 and self.use_sl_tp:
                    ...
                    if current_sl is not None:
                        if (position > 0 and current_price <= current_sl) or (
                            position < 0 and current_price >= current_sl
                        ):
                            # SL atteint, fermer la position
                            trade_pnl = (
                                position * (current_sl - entry_price) - abs(position * current_sl) * self.commission
                            )
                            capital += trade_pnl
                    ...
                    # Vérifier si le prix a atteint le TP
                        ...
                        if (position > 0 and current_price >= current_tp) or (
                            position < 0 and current_price <= current_tp
                        ):
                            # TP atteint, fermer la position
                            trade_pnl = (
                                position * (current_tp - entry_price) - abs(position * current_tp) * self.commission
                            )
                            capital += trade_pnl
                    ...
                    if position <= 0:  # Si pas de position ou short, entrer en long
                        ...
                        if position < 0:
                            trade_pnl = (
                                ...
                                - abs(position * current_price) * self.commission
                            )
                            capital += trade_pnl
            ...
            equity_curve.append({"timestamp": current_time, "equity": equity, "position": position})
