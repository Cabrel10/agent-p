...
class BacktestVisualizer:
    ...
    def plot_drawdown(self, output_path: Optional[Path] = None) -> go.Figure:
        ...
        drawdown_df = pd.DataFrame({"timestamp": self.equity_curve["timestamp"], "drawdown": drawdown})
    ...
    def plot_trades_distribution(self, output_path: Optional[Path] = None) -> go.Figure:
        ...
        fig.add_trace(
            go.Histogram(x=self.trades["pnl"], name="P&L", marker_color=self.colors["equity"], opacity=0.7, nbinsx=20),
            row=1,
            col=1,
        )
        ...
        # Configurer le layout
        fig.update_layout(title="Distribution des Trades", template="plotly_white", showlegend=False, height=800)
    ...
    def plot_monthly_returns(self, output_path: Optional[Path] = None) -> go.Figure:
        ...
        monthly_returns = (
            daily_equity.groupby("year_month")
            .apply(lambda x: (x["equity"].iloc[-1] / x["equity"].iloc[0]) - 1)
            .reset_index()
        )
    ...
    def plot_trade_analysis(self, output_path: Optional[Path] = None) -> go.Figure:
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "pie"}, {"type": "pie"}], [{"type": "bar"}, {"type": "scatter"}]],
            vertical_spacing=0.2,
        ...
        fig.add_trace(
            go.Pie(labels=exit_reasons.index, values=exit_reasons.values, textinfo="percent+label", hole=0.4),
            row=1,
            col=2,
        )
        ...
        self.trades["duration_category"] = pd.cut(
            self.trades["duration_hours"], bins=duration_bins, labels=duration_labels
        )

        duration_counts = self.trades["duration_category"].value_counts().sort_index()

        fig.add_trace(
            go.Bar(x=duration_counts.index, y=duration_counts.values, marker_color=self.colors["equity"], opacity=0.7),
            row=2,
            col=1,
        )
        ...
        # Configurer le layout
        fig.update_layout(title="Analyse des Trades", template="plotly_white", showlegend=False, height=800)
