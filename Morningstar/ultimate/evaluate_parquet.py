...
def parse_args():
    ...
    parser.add_argument("--model-path", type=str, required=True, help="Chemin vers le modèle entraîné (.h5)")
    parser.add_argument("--data-path", type=str, required=True, help="Chemin vers le dataset (format parquet)")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument("--optimize", action="store_true", help="Effectuer une optimisation après l'évaluation")
    parser.add_argument("--learning-rate", type=float, default=0.0001, help="Taux d'apprentissage pour l'optimisation")
    parser.add_argument("--epochs", type=int, default=5, help="Nombre d'époques pour l'optimisation")
...
def evaluate_model(model, X, y_signal, y_regime):
    ...
    signal_report = classification_report(
        true_signal_classes, signal_classes, target_names=["Achat", "Neutre", "Vente"], output_dict=True
    )
    ...
    logger.info(classification_report(true_signal_classes, signal_classes, target_names=["Achat", "Neutre", "Vente"]))

    logger.info("\nRapport de classification pour le régime de marché:")
    logger.info(
        classification_report(
            true_regime_classes, regime_classes, target_names=["Baissier", "Neutre", "Haussier", "Volatil"]
        )
    )
...
def optimize_model(model, X, y_signal, y_regime, learning_rate=0.0001, epochs=5):
    ...
    model.compile(
        ...
        loss={"signal": "categorical_crossentropy", "regime": "categorical_crossentropy"},
        metrics={"signal": ["accuracy"], "regime": ["accuracy"]},
    )
...
def main():
    ...
    if args.optimize:
        optimized_model, history = optimize_model(
            model, X, y_signal, y_regime, learning_rate=args.learning_rate, epochs=args.epochs
        )
