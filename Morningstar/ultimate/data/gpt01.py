...
if __name__ == "__main__":
    ...
    for symbol in symbols:
        ...
        snippets_map = {date: get_news_snippets(f"{symbol} crypto market news {date}", cache) for date in unique_dates}
        ...
        df[["mcp_1", "mcp_2", "mcp_3"]] = pca.fit_transform(df[["close", "volume", "market_regime"]])
        all_dfs.append(df)
    if not all_dfs:
