...
class MarketInfoCollector:
    ...
    def get_coinmarketcap_key(self):
        ...
        sorted_keys = sorted(
            self.coinmarketcap_api_keys, key=lambda k: (self.cmc_key_errors.get(k, 0), self.cmc_key_usage.get(k, 0))
        )
    ...
    def get_google_key(self):
        ...
        # Trier les clés par nombre d'erreurs puis par nombre d'utilisations
        sorted_keys = sorted(
            self.google_api_keys, key=lambda k: (self.google_key_errors.get(k, 0), self.google_key_usage.get(k, 0))
        )
    ...
    def get_crypto_metadata(self, symbol):
        ...
        try:
            ...
            if response.status_code == 200 and "data" in data:
                ...
                logger.error(
                    f"Erreur lors de la récupération des métadonnées pour {symbol}: {data.get('status', {}).get('error_message', 'Erreur inconnue')}"
                )
    ...
    def get_market_metrics(self, symbol):
        ...
        try:
            ...
            if response.status_code == 200 and "data" in data:
                ...
                logger.error(
                    f"Erreur lors de la récupération des métriques pour {symbol}: {data.get('status', {}).get('error_message', 'Erreur inconnue')}"
                )
                self.cmc_key_errors[api_key] = self.cmc_key_errors.get(api_key, 0) + 1
                return {}
        except Exception as e:
    ...
    def get_crypto_news(self, symbol, limit=10):
        ...
        try:
            ...
            if response.status_code == 200 and "items" in data:
                ...
                logger.error(
                    f"Erreur lors de la récupération des actualités pour {symbol}: {data.get('error', {}).get('message', 'Erreur inconnue')}"
                )
    ...
    def get_global_market_data(self):
        ...
        try:
            ...
            if response.status_code == 200 and "data" in data:
                ...
                logger.error(
                    f"Erreur lors de la récupération des données globales du marché: {data.get('status', {}).get('error_message', 'Erreur inconnue')}"
                )
                self.cmc_key_errors[api_key] = self.cmc_key_errors.get(api_key, 0) + 1
                return {}
        except Exception as e:
...
def initialize_market_info_collector():
    ...
    collector = MarketInfoCollector(coinmarketcap_api_keys=coinmarketcap_api_keys, google_api_keys=google_api_keys)
