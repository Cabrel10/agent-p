...
class CryptoNewsCollector:
    ...
    def _fetch_rss_news(
        self, rss_url: str, target_date: datetime, crypto_name: Optional[str] = None
    ) -> List[Dict[str, str]]:
        """
            ...
            try:
                ...
                for item in items:
                    try:
                        ...
                        if (
                            crypto_name
                            and crypto_name.lower() not in title.lower()
                            and crypto_name.lower() not in description.lower()
                        ):
                            continue
                ...
                logger.error(
                    f"Erreur lors de la ru00e9cupu00e9ration du flux RSS (tentative {attempt+1}/{self.max_retries}): {e}"
                )
                time.sleep(self.retry_delay)
    ...
    def _fetch_api_news(
        self, api_url: str, target_date: datetime, crypto_name: Optional[str] = None
    ) -> List[Dict[str, str]]:
        """
            ...
            try:
                ...
                for article in articles:
                    try:
                        ...
                        news.append(
                            {"title": title, "content": content, "source": source, "date": date_str, "url": url}
                        )
                ...
                logger.error(
                    f"Erreur lors de la ru00e9cupu00e9ration des actualitu00e9s API (tentative {attempt+1}/{self.max_retries}): {e}"
                )
                time.sleep(self.retry_delay)
    ...
    def _fetch_article_content(self, url: str) -> Optional[str]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"Erreur lors de la ru00e9cupu00e9ration du contenu de l'article (tentative {attempt+1}/{self.max_retries}): {e}"
                )
                time.sleep(self.retry_delay)
