...
class GeminiSentimentAnalyzer:
    ...
    def _initialize_clients(self):
        ...
        for api_key in self.api_keys:
            try:
                ...
                logger.error(
                    f"Erreur lors de l'initialisation du client Gemini pour la clé se terminant par ...{api_key[-4:]}: {e}"
                )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Analyse de sentiment réussie pour {crypto_name} avec la clé se terminant par ...{api_key[-4:]}"
                    )
                    return result
                ...
                logger.error(
                    f"Erreur lors de l'analyse de sentiment (tentative {attempt+1}/{self.max_retries}) avec la clé se terminant par ...{api_key[-4:]}: {e}"
                )
                self.key_errors[api_key] += 1
                ...
                if "quota" in str(e).lower() or "limit" in str(e).lower() or "invalid" in str(e).lower():
                    logger.warning(
                        f"La clé API se terminant par ...{api_key[-4:]} semble épuisée ou invalide. Essai d'une autre clé."
                    )
                    api_key, client = self.get_client()
                    if not client:
                        break
    ...
    def analyze_market_context(self, price_data: Dict[str, Any], news_summary: str, crypto_name: str) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"Erreur lors de l'analyse du contexte de marché (tentative {attempt+1}/{self.max_retries}): {e}"
                )
                time.sleep(self.retry_delay)
    ...
    def generate_trading_insights(
        self, market_data: Dict[str, Any], sentiment_data: Dict[str, Any], crypto_name: str
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"Erreur lors de la génération des insights de trading (tentative {attempt+1}/{self.max_retries}): {e}"
                )
                time.sleep(self.retry_delay)
