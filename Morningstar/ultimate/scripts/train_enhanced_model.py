...
def parse_args():
    ...
    parser.add_argument("--batch-size", type=int, default=32, help="Taille du batch pour l'entrau00eenement")
    parser.add_argument("--use-llm", action="store_true", help="Utiliser les embeddings LLM si disponibles")
    parser.add_argument("--use-hmm", action="store_true", help="Utiliser les features HMM")
...
def prepare_data(df, use_llm=False, use_hmm=True):
    ...
    if use_llm and cryptobert_features:
        ...
        logger.warning(
            "Embeddings CryptoBERT non disponibles ou non utilisu00e9s, utilisation de vecteurs de zu00e9ros"
        )
        train_llm = np.zeros((len(train_df), 768))
        val_llm = np.zeros((len(val_df), 768))
        test_llm = np.zeros((len(test_df), 768))
    ...
    train_outputs = {"signal": train_signal, "market_regime": train_regime, "sl_tp": train_sl_tp}

    val_outputs = {"signal": val_signal, "market_regime": val_regime, "sl_tp": val_sl_tp}

    test_outputs = {"signal": test_signal, "market_regime": test_regime, "sl_tp": test_sl_tp}
...
def build_model(tech_input_shape, use_llm=False):
    ...
    model.compile(
        ...
        loss={"signal": "categorical_crossentropy", "market_regime": "categorical_crossentropy", "sl_tp": "mse"},
        metrics={"signal": ["accuracy"], "market_regime": ["accuracy"], "sl_tp": ["mae"]},
        loss_weights={"signal": 1.0, "market_regime": 1.0, "sl_tp": 0.5},
    )
...
def main():
    ...
    test_results = model.evaluate(data["test_inputs"], data["test_outputs"], verbose=1)
