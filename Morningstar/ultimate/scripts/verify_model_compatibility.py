...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="reports", help="Répertoire de sortie pour le rapport de compatibilité"
    )
...
def generate_compatibility_report(dataset_dims, compatibility_results, output_dir):
    ...
    report = {
        ...
        "model_expected_dimensions": {"tech_dim": 38, "llm_dim": 768, "mcp_dim": 128, "hmm_dim": 4},
        "compatibility": {
    ...
    if not compatibility_results["overall_compatible"]:
        ...
        if compatibility_results["tech_adjustment"]:
            example_content.append(
                f"    tech_input_shape=({dataset_dims['tech_dim']},),  # Ajustement pour les features techniques"
            )
        if compatibility_results["llm_adjustment"]:
            example_content.append(
                f"    llm_embedding_dim={dataset_dims['llm_dim']},  # Ajustement pour les embeddings LLM"
            )
...
def main():
    ...
    if compatibility_results["overall_compatible"]:
        ...
        logger.warning(
            "Le modèle n'est pas compatible avec le dataset. Veuillez consulter le rapport pour les ajustements nécessaires."
        )
