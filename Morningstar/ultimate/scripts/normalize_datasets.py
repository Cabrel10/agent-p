...
def stratified_split_dataset(df, test_size=0.15, val_size=0.15, stratify_col="market_regime", random_state=42):
    ...
    train_val_df, test_df = train_test_split(df, test_size=test_size, random_state=random_state, stratify=stratify)
...
def standardize_model_output():
    ...
    metadata = {
        ...
        "input_features": {"technical_indicators": 38, "hmm_features": 4, "llm_features": 2, "mcp_features": 128},
        "output_features": {
            "trading_signal": 5,
            "market_regime": 4,
    ...
    logger.info(
        f"Métadonnées du modèle sauvegardées dans {standardized_model_dir / f'{standard_model_name}_metadata.json'}"
    )
