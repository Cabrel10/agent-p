...
from app_modules.utils import BASE_DIR, DATA_DIR, MODEL_DIR, REPORTS_DIR, load_dataset
...
def parse_args():
    ...
    parser.add_argument(
        "--num-market-regime-classes", type=int, default=4, help="Nombre de classes de régime de marché"
    )
...
def create_morningstar_model(args):
    ...
    if args.use_chain_of_thought:
        ...
        attention_output = MultiHeadAttention(num_heads=args.num_attention_heads, key_dim=16)(x, x)
...
def prepare_data(dataset_path, args):
    ...
    if len(tech_cols) < args.tech_input_dim:
        print(
            f"Attention: Le nombre de colonnes techniques ({len(tech_cols)}) est inférieur à la dimension spécifiée ({args.tech_input_dim})"
        )
        ...
        print(
            f"Attention: Le nombre de colonnes techniques ({len(tech_cols)}) est supérieur à la dimension spécifiée ({args.tech_input_dim})"
        )
    ...
    if len(llm_cols) < args.llm_embedding_dim:
        print(
            f"Attention: Le nombre de colonnes LLM ({len(llm_cols)}) est inférieur à la dimension spécifiée ({args.llm_embedding_dim})"
        )
        # Compléter avec des zéros si nécessaire
        ...
        print(
            f"Attention: Le nombre de colonnes LLM ({len(llm_cols)}) est supérieur à la dimension spécifiée ({args.llm_embedding_dim})"
        )
    ...
    if len(mcp_cols) < args.mcp_input_dim:
        print(
            f"Attention: Le nombre de colonnes MCP ({len(mcp_cols)}) est inférieur à la dimension spécifiée ({args.mcp_input_dim})"
        )
        # Compléter avec des zéros si nécessaire
        ...
        print(
            f"Attention: Le nombre de colonnes MCP ({len(mcp_cols)}) est supérieur à la dimension spécifiée ({args.mcp_input_dim})"
        )
    ...
    if len(hmm_cols) < args.hmm_input_dim:
        print(
            f"Attention: Le nombre de colonnes HMM ({len(hmm_cols)}) est inférieur à la dimension spécifiée ({args.hmm_input_dim})"
        )
        # Compléter avec des zéros si nécessaire
        ...
        print(
            f"Attention: Le nombre de colonnes HMM ({len(hmm_cols)}) est supérieur à la dimension spécifiée ({args.hmm_input_dim})"
        )
    ...
    if "trading_signal" in df.columns:
        ...
        if trading_signal.shape[1] != args.num_trading_classes:
            print(
                f"Attention: Le nombre de classes de trading ({trading_signal.shape[1]}) ne correspond pas à la valeur spécifiée ({args.num_trading_classes})"
            )
    ...
    if "market_regime" in df.columns:
        ...
        if market_regime.shape[1] != args.num_market_regime_classes:
            print(
                f"Attention: Le nombre de classes de régime ({market_regime.shape[1]}) ne correspond pas à la valeur spécifiée ({args.num_market_regime_classes})"
            )
            # Ajuster le nombre de classes si nécessaire
            if market_regime.shape[1] < args.num_market_regime_classes:
    ...
    if len(volatility_cols) == args.num_volatility_quantiles:
        ...
        print(
            f"Attention: Le nombre de colonnes de volatilité ({len(volatility_cols)}) ne correspond pas à la valeur spécifiée ({args.num_volatility_quantiles})"
        )
    ...
    if len(sl_tp_cols) == args.num_sl_tp_outputs:
        ...
        print(
            f"Attention: Le nombre de colonnes SL/TP ({len(sl_tp_cols)}) ne correspond pas à la valeur spécifiée ({args.num_sl_tp_outputs})"
        )
    ...
    feature_scaler = {
        ...
        "instrument_input": {
            "mean": X["instrument_input"].mean(axis=0),
            "std": X["instrument_input"].std(axis=0) + 1e-8,
        },
    }
    ...
    y["sl_tp_values"] = (y["sl_tp_values"] - target_scaler["sl_tp_values"]["mean"]) / target_scaler["sl_tp_values"][
        "std"
    ]
...
def main():
    ...
    callbacks.append(
        ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=args.early_stopping_patience // 2, min_lr=1e-6)
    )

    # TensorBoard
    callbacks.append(TensorBoard(log_dir=str(logs_dir), histogram_freq=1))
    ...
    if args.save_metadata:
        metadata = {
            ...
            "history": {key: [float(val) for val in history.history[key]] for key in history.history.keys()},
        }
