...
def parse_args():
    ...
    parser.add_argument(
        "--input-dir", type=str, default="data/processed", help="Répertoire contenant les datasets prétraités"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument("--test-size", type=float, default=0.15, help="Proportion de données pour le test")
    parser.add_argument("--val-size", type=float, default=0.15, help="Proportion de données pour la validation")
    parser.add_argument("--random-state", type=int, default=42, help="Seed pour la reproductibilité")
    parser.add_argument("--stratify-col", type=str, default="market_regime", help="Colonne pour la stratification")
    parser.add_argument(
        "--feature-config",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--output-name", type=str, default="standardized_multi_crypto_dataset", help="Nom du dataset standardisé"
    )
...
def main():
    ...
    if feature_config:
        ...
        feature_columns = [col for col in sample_df.columns if col not in label_columns + sl_tp_cols + [asset_id_col]]

        # Détecter les colonnes techniques, LLM, MCP et HMM
        technical_cols = [
            col
            for col in feature_columns
            if col.startswith(("rsi_", "macd_", "bbands_", "ema_", "sma_", "atr_", "adx_"))
            or col in ["open", "high", "low", "close", "volume", "returns"]
        ]

        llm_cols = [col for col in feature_columns if col.startswith("cryptobert_dim_")]
        mcp_cols = [col for col in feature_columns if col.startswith(("market_", "global_", "sentiment_"))]
