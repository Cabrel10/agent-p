...
def parse_args():
    ...
    parser.add_argument(
        "--dataset-path", type=str, required=True, help="Chemin vers le dataset normalisé (format parquet)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument("--epochs", type=int, default=30, help="Nombre d'époques d'entraînement")
    parser.add_argument("--batch-size", type=int, default=64, help="Taille du batch pour l'entraînement")
    parser.add_argument(
        "--validation-split", type=float, default=0.2, help="Proportion des données à utiliser pour la validation"
    )
    parser.add_argument(
        "--early-stopping", action="store_true", help="Utiliser l'early stopping pour éviter le surapprentissage"
    )
...
def create_targets(df, n_samples):
    ...
    if "target_signal" in df.columns:
        ...
        y_regime = (
            df["target_regime"].values if "target_regime" in df.columns else np.random.randint(0, 4, size=n_samples)
        )
    else:
...
def build_simple_model(input_dim):
    ...
    # Têtes de sortie
    signal_output = tf.keras.layers.Dense(3, activation="softmax", name="signal")(
        shared
    )  # 3 classes: Vendre, Conserver, Acheter
...
def train_model(model, X, y_signal, y_regime, args):
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath=str(checkpoint_path), save_best_only=True, monitor="val_loss", mode="min", verbose=1
    )
...
def save_model(model, output_dir, features):
    ...
    metadata = {
        ...
        "output_classes": {"signal": 3, "regime": 4},  # Vendre, Conserver, Acheter  # 4 régimes de marché
    }
