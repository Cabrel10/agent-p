        ...
        if hasattr(reasoning_module, "ReasoningModule"):
            ...
            for idx, row in sample_data.iterrows():
                try:
                    ...
                    explanations.append(
                        {
                            ...
                            "input_data": {
                                ...
                                if k
                                in ["symbol", "open", "high", "low", "close", "volume", "RSI", "MACD", "hmm_regime"]
                            },
                    ...
                    logger.error(
                        f"Erreur lors de la gu00e9nu00e9ration de l'explication pour l'u00e9chantillon {idx}: {e}"
                    )
            ...
            logger.error(
                "Le module de raisonnement ne contient pas de classe ReasoningModule ou de fonction generate_explanation"
            )
            return None
    except Exception as e:
...
def generate_example_explanations():
    ...
    for idx, row in sample_data.iterrows():
        ...
        if hmm_regime == 1 and rsi < 70 and macd > 0:
            signal = "ACHETER"
            reason = (
                "Le marchu00e9 est dans un ru00e9gime haussier, le RSI n'est pas en surachat, et le MACD est positif."
            )
        ...
        explanations.append(
            {
                "sample_idx": idx,
                "input_data": {"symbol": symbol, "close": close, "RSI": rsi, "MACD": macd, "hmm_regime": hmm_regime},
                "explanation": explanation,
            }
        )
...
def main():
    ...
    if reasoning_module is not None and model is not None and sample_data is not None:
        ...
        if explanations is None or len(explanations) == 0:
            logger.warning(
                "Aucune explication gu00e9nu00e9ru00e9e par le module de raisonnement, gu00e9nu00e9ration d'exemples..."
            )
