...
def prepare_improved_dataset(input_path, output_path=None, test_size=0.2, random_state=42):
    ...
    if "market_regime" in train_df.columns:
        ...
        # Identifier les features NUMÉRIQUES et la cible pour SMOTE
        # Exclure la cible et les colonnes non numériques ou identifiants
        feature_cols_for_smote = [
            col
            for col in train_df.columns
            if col not in ["market_regime", "split", "symbol", "timestamp"]  # Exclure cible et identifiants
            and pd.api.types.is_numeric_dtype(train_df[col])  # Garder seulement les numériques
        ]

        if not feature_cols_for_smote:
            logger.warning("Aucune colonne de feature numérique trouvée pour SMOTE. Skipping SMOTE.")
        else:
            X_train = train_df[feature_cols_for_smote]
        ...
        try:
            ...
            train_df_resampled = pd.DataFrame(X_train_resampled, columns=feature_cols_for_smote)
            train_df_resampled["market_regime"] = y_train_resampled

            # Ajouter les colonnes non-numériques qui ont été exclues (si nécessaire, mais peut être complexe)
            # Pour l'instant, on continue avec le DataFrame resamplé contenant uniquement les features numériques + target
...
def main():
    parser = argparse.ArgumentParser(
        description="Pru00e9pare un dataset amélioré avec normalisation robuste et u00e9quilibrage des classes."
    )
    ...
    parser.add_argument(
        "--test-size", type=float, default=0.2, help="Proportion des donnu00e9es u00e0 utiliser pour le test"
    )
    parser.add_argument(
        "--random-state", type=int, default=42, help="Graine alu00e9atoire pour la reproductibilitu00e9"
    )
