...
def fetch_gemini_context(symbol: str, date_str: str, api_key: Optional[str]) -> str:
    """
    Placeholder for fetching context from Gemini.
    In a real scenario, this would interact with the Gemini API or a data source.
    """
    logger.info(f"Fetching Gemini context for {symbol} on {date_str} (mocked).")
    # Simuler un contexte basé sur la date pour la reproductibilité des tests
    # Convertir date_str en objet datetime pour obtenir le jour de l'année
    try:
        date_obj = datetime.strptime(date_str, "%Y-%m-%d")
        day_of_year = date_obj.timetuple().tm_yday
    except ValueError:
        day_of_year = 1  # Valeur par défaut

    if day_of_year % 3 == 0:
        return f"Positive sentiment for {symbol} on {date_str} due to major product launch."
    elif day_of_year % 3 == 1:
        return f"Neutral sentiment for {symbol} on {date_str}, market is consolidating."
    else:
        return f"Negative sentiment for {symbol} on {date_str} following regulatory concerns."
...
GEMINI_API_BACKUP_KEYS = ["AIzaSyCH9ocKRxb_4l7AcdKbzPJAAV1xZ1s-tMQ", "AIzaSyAW7_-kTU1EH8dMXv0oBeDt49mH6lmkZhg"]
...
def main():
    ...
    try:
        ...
        df = api_manager.fetch_ohlcv_data(args.exchange, args.symbol, args.interval, args.start_date, args.end_date)
