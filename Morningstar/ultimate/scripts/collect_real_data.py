        ...
        for symbol in tqdm(self.symbols, desc="Collecte des donnu00e9es de marchu00e9"):
            try:
                ...
                market_data = self.market_collector.fetch_ohlcv(
                    symbol=symbol, timeframe=self.timeframe, since=self.start_date, until=self.end_date
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/real",
        help="Ru00e9pertoire de sortie pour les donnu00e9es collectu00e9es",
    )
    ...
    if not final_dataset.empty:
        logger.info(
            f"Dataset final gu00e9nu00e9ru00e9 avec succu00e8s: {len(final_dataset)} lignes, {len(final_dataset.columns)} colonnes"
        )
    else:
