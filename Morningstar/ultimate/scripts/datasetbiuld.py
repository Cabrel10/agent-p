    ...
    def download_crypto_data(self, pair: str) -> pd.DataFrame:
        ...
        try:
            ...
            date_range = pd.date_range(
                start=start, end=end, freq="h" if self.interval == "1h" else "4h" if self.interval == "4h" else "D"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=str, default="1h", choices=["1h", "4h", "1d"], help="Intervalle temporel (1h, 4h, 1d)"
    )
    parser.add_argument("--output-dir", type=str, default="data/processed", help="Répertoire de sortie du dataset")
    parser.add_argument("--pairs", type=str, nargs="+", help="Liste des paires à inclure (ex: BTC/USDT ETH/USDT)")
