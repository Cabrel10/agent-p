...
def parse_args():
    ...
    parser.add_argument("--end-date", type=str, default="2023-01-31", help="Date de fin au format YYYY-MM-DD")
    parser.add_argument(
    ...
    parser.add_argument("--max-workers", type=int, default=4, help="Nombre maximum de workers pour le multithreading")
    parser.add_argument(
        "--use-cache", action="store_true", default=True, help="Utiliser le cache pour accélérer la collecte"
    )
...
def collect_market_info(symbols, use_cache=True, max_workers=4):
    ...
    def collect_info_for_symbol(symbol):
        ...
        try:
            ...
            return base_symbol, {"metadata": metadata, "metrics": metrics}
        except Exception as e:
...
def enrich_dataframe(market_data, news_data, sentiment_data, cryptobert_data, hmm_data, market_info_data):
    ...
    embedding_dim = (
        10  # Dimension des embeddings CryptoBERT (réduite pour éviter de trop augmenter la taille du dataset)
    )
...
def main():
    ...
    if args.use_sentiment and news_data:
        sentiment_data = analyze_sentiment(news_data=news_data, use_cache=args.use_cache, max_workers=args.max_workers)
    ...
    if args.use_cryptobert and news_data:
        cryptobert_data = generate_cryptobert_embeddings(
            news_data=news_data, use_cache=args.use_cache, max_workers=args.max_workers
        )

    # Détecter les régimes HMM si demandé
    if args.use_hmm and not market_data.empty:
        hmm_data = detect_hmm_regimes(market_data=market_data, use_cache=args.use_cache, max_workers=args.max_workers)

    # Collecter les informations de marché si demandé
    if args.use_market_info:
        market_info_data = collect_market_info(
            symbols=args.symbols, use_cache=args.use_cache, max_workers=args.max_workers
        )
